        ...
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <p className="text-sm font-semibold leading-tight text-text">Sciember</p>
...
export function MobileAppSidebar({ onClose }: { onClose: () => void }) {
  const { data: session, status } = useSession();
  const { openAuthModal } = useAuthModal();
  const router = useRouter();
  const [score, setScore] = useState<number | null>(null);
  ...
  return (
    <aside className="flex h-full w-[min(86vw,320px)] flex-col border-r border-border bg-surface">
      <div className="flex items-center justify-between gap-2 border-b border-border p-4">
        ...
        <button
          type="button"
          onClick={onClose}
          className="inline-flex h-9 w-9 items-center justify-center rounded-xl border border-border bg-card/80 text-text shadow-sm backdrop-blur-sm transition hover:border-primary/40 hover:bg-primary-soft/50"
